...
앱스토어 심사에서 온 것들(`MVP.md` 「앱스토어 심사 요건」). 기능이 아니라 자격 요건이다.
...
- 유저 행, 멤버십, 기상 기록, 목소리 파일까지 **전부** 삭제
- 방장인 방이 있으면: 다른 멤버가 있는 방은 참여 순서가 가장 빠른 멤버에게 이관, 혼자인 방은 삭제
- 심사 기준 5.1.1(v) — 계정을 만들 수 있으면 지울 수도 있어야 함

### `POST /me/blocks/{userId}` · `DELETE /me/blocks/{userId}` — 차단/해제

```
→ 204
```

- 차단하면 **배정에서 양방향 제외**: 그 사람 목소리가 나를 깨우지 않고, 내 목소리도 그 사람을
  깨우지 않음
- 이미 만들어진 배정은 그대로 둠 (§2의 고정 규칙) — 다음 배정부터 반영
- `block(user_id, blocked_id)` 테이블, PK 복합

### `POST /reports` — 신고 접수

```jsonc
{ "memberId": "usr_B", "roomId": "rm_1" }
→ 201
```

- 저장하고 운영자가 볼 수 있으면 v1로는 충분. 자동 조치 없음
- 심사 기준 1.2 — 신고 수단이 있고 개발자가 대응한다는 것
...
심사 요건으로 `block(user_id, blocked_id)`(복합 PK)과 `report(id, member_id, room_id, at)`
테이블이 추가된다.
...
로그인(SIWA) · 푸시(`/me/devices`) · 오디오 변환 · 알람 스케줄링 · 스누즈 처리 · 크론/배치.
