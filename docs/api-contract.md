...
iOS 클라이언트가 서버에 요구하는 것. 지금 있는 클라이언트를 기준으로 썼고, 모양의 근거는
`frontend/lib/model.ts`다. `frontend/lib/store.ts`가 이 엔드포인트들이 대체할 자리 — 처음부터 async에
API 모양이라 **fetch로 바꿔도 화면은 한 줄도 안 건드린다.**

> 이 문서의 클라이언트 경로는 모두 **`frontend/`** 기준이다.
>
> - `docs/MVP.md` — **v1에 무엇이 들어가는지.** 이 문서와 어긋나면 그쪽이 이긴다.
> - `docs/openapi.yaml` — 같은 계약의 기계용 버전. lint 통과, 목서버·코드젠에 바로 쓸 수 있다.
> - 이 문서 — **왜 그런 모양인지.**

**어디서부터 만들지 모르겠으면 §9 「구현 순서」부터 읽으면 된다.**

---

## 1. 먼저 읽을 것: 서버는 아무것도 울리지 않는다

알람은 **기기에서** iOS AlarmKit이 울린다. 폰이 오프라인이든, 기내 모드든, 앱이 죽어 있든 울린다.
서버는 울리는 경로에 **절대** 없고, 누군가를 깨우는 일을 서버에 의존해서는 안 된다.

그래서 서버가 답해야 하는 질문은 세 개뿐이다.

1. **이 방에 누가 있고** 각자 몇 시에 일어나는지
2. **내일 아침 누구 목소리가 울리는지**, 그 오디오 파일은 어디 있는지
3. **누가 실제로 일어났는지** — 공유되는 모닝 로그

나머지는 전부 로컬이다. 특히 **알람 스케줄링, 스누즈 처리, "07:00에 알림 보내기"를 만들지 말 것.**
클라이언트가 이미 다 한다.
...
| 필드 | 이유 |
| --- | --- |
| `Room.nativeAlarmId` | **이 기기에서의** AlarmKit 알람 id. 다른 어디서도 의미가 없다. |
| `Member.voiceSoundName` | **이 기기에서의** 오디오 파일명. 서버는 URL을 주고, 파일명은 클라이언트가 정한다. |
| `PersonalAlarm.*` | 방 없는 알람. 완전히 로컬이고 폰을 떠나지 않는다. |
